...
export class DirectAccessService {
  ...
  constructor(
    private readonly http: HttpProbeService,
    private readonly attackTests: AttackTestsService,
    private readonly filePathScan: FilePathScanService,
    private readonly bannerSvc: BannerService,
    private readonly resultBuilder: ResultBuilderService,
  ) {}
  ...
  async test(
    ...
    // ── Run all test categories sequentially with jitter between them ────────
    const basicAccess = await this.attackTests.testBasicAccess(ip, domain);
    await this.http.sleepJitter(200, 150);

    const hostVariants = await this.attackTests.testHostVariants(ip, domain);
    await this.http.sleepJitter(200, 150);

    const xffBypass = await this.attackTests.testXFFBypass(ip, domain);
    await this.http.sleepJitter(200, 150);

    const cfSpoof = await this.attackTests.testCFHeaderSpoof(ip, domain);
    await this.http.sleepJitter(200, 150);

    const sniMismatch = await this.attackTests.testSNIMismatch(ip, domain);
    await this.http.sleepJitter(200, 150);

    const exposedFiles = await this.filePathScan.testExposedFiles(
      ip,
      domain,
      mode,
    );
    ...
    const knownPaths = await this.filePathScan.testKnownPaths(ip, domain, mode);
    await this.http.sleepJitter(200, 150);

    const banners = await this.bannerSvc.grabAll(ip, openPorts);
