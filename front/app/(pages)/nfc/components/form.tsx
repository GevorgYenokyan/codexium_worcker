...
import { useGetUserByIdQuery, useUpdateNfcMutation } from "@/app/redux/features/api/codexiumApi";
...
const NfcForm: FC = () => {
    ...
    const [update, { isLoading, isSuccess, isError, error }] = useUpdateNfcMutation();
    ...
    useEffect(() => {
        if (data?.nfc && id) {
            const nfcItem = data.nfc.find((item: any) => String(item.id) === id);
            if (nfcItem) {
                setValues((prev) => ({
                    ...prev,
                    // path: nfcItem.path || "",
                    name: nfcItem.name || "",
                    company_name: nfcItem.company_name || "",
                    type: nfcItem.type || "",
                    profession: nfcItem.profession || "",
                    description: nfcItem.description || "",
                    companies: nfcItem.companies || "",
                    website: nfcItem.website || "",
                    phone: nfcItem.phone || "",
                    facebook: nfcItem.facebook || "",
                    linkedin: nfcItem.linkedin || "",
                    position: nfcItem.position || "",
                    instagram: nfcItem.instagram || "",
                    behance: nfcItem.behance || "",
                    viber: nfcItem.viber || "",
                    telegram: nfcItem.telegram || "",
                    email: nfcItem.email || "",
                    links: nfcItem.links || "",
                    font: nfcItem.font || "",
                }));

                setPone(nfcItem?.phone?.trim());
                setWhatsapp(nfcItem?.whatsapp?.trim());

                if (nfcItem?.image) {
                    setImageUrl([
                        {
                            id: Math.random(),
                            src: `https://codexium.it/${nfcItem?.image}`,
                        },
                    ]);
                ...
                <Form className={classes.main_form} fields={[]} onSubmit={(e) => {}}>
                    ...
                    <div className={classes.designOptions}>
                        {[
                            ...
                            { id: 2, title: "Classic", desc: "Company Name" },
                            { id: 3, title: "Minimal", desc: "Company Name" },
                            { id: 4, title: "Elegant", desc: "Company Name" },
                        ].map((el) => {
                            return (
                                <div
